...
public class PktDecoder implements StdEntropyCoderOptions
{
    ...
    protected final static int NOT_READY = Integer.MIN_VALUE;
     ...
     * </ul>
     * */
    private PrecInfo[][][] ppinfo; // [comp][res][precinct]
    private int[][][] nextLayer; // [comp][res][precinct]
     ...
     *
     * */
    public PktDecoder(DecoderSpecs decSpec, HeaderDecoder hd, BitstreamReaderAgent src, boolean isTruncMode, int maxCB)
    {
     ...
     * */
    public CBlkInfo[][][][][] restart(int nc, int[] mdl, int nl, boolean pph, ByteArrayInputStream pphbais)
    {
        this.nc = nc;
        this.nl = nl;
        ...
        int mins, maxs;
        Coord nBlk = null;
        int cb0x = hd.getCbULX();
        int cb0y = hd.getCbULY();
        ...
        for (int c = 0; c < nc; c++)
        {
            ...
            for (int r = 0; r <= mdl[c]; r++)
            {
                ...
                if (trx1 > trx0)
                {
                    numPrec[c][r].x = (int) Math.ceil((trx1 - cb0x) / twoppx) - (int) Math.floor((trx0 - cb0x) / twoppx);
                }
                else
                {
                    numPrec[c][r].x = 0;
                ...
                if (try1 > try0)
                {
                    numPrec[c][r].y = (int) Math.ceil((try1 - cb0y) / twoppy) - (int) Math.floor((try0 - cb0y) / twoppy);
                }
                else
                {
                    numPrec[c][r].y = 0;
     ...
     * */
    public boolean readPktHead(ByteArrayInputStream in, int l, int r, int c, int p, CBlkInfo[][][] cbI, int[] nb) throws IOException
    {
        ...
        SubbandSyn sb;
        SubbandSyn root = src.getSynSubbandTree(tIdx, c);
        l = nextLayer[c][r][p]; // get layer index
                ...
                { // Horizontal code-blocks
                    ...
                    try
                    {
                        ...
                        if (ccb.ctp == 0)
                        {
                            ...
                            //System.out.print("Bit depth = " + (tmp2-2));
                        ...
                        { // If code-block already included in one of
                            ...
                            if (bin.readBit() != 1)
                            {
                                System.out.println("  l: " + l + " - s: " + s + " - ccb: " + cbc.x + "," + cbc.y + " skipped");
                                continue;
                            }
                        ...
                        ccb.len[l] = cbLen;
                        //System.out.println("  ccb " + cbc.x + "," + cbc.y + ": length=" + cbLen);
        ...
        pktIdx++;
        nextLayer[c][r][p]++; // set next layer
     ...
     * */
    public boolean readPktBodyOffsets(ByteArrayInputStream in, int l, int r, int c, int p, CBlkInfo[][][] cbI, int[] nb) throws IOException
    {
        int curOff = 0;
        Coord curCB;
        CBlkInfo ccb;
        ...
        for (int s = mins; s < maxs; s++)
        {
            for (int numCB = 0; numCB < cblks[s].size(); numCB++)
            {
                ...
                ccb.off[l] = curOff;
                curOff += ccb.len[l];
                long skipped = in.skip(ccb.len[l]);
                
                // if eof
                if (skipped != ccb.len[l])
                {
                    if (l == 0)
                    {
                        cbI[s][cbc.y][cbc.x] = null;
...
//                }
     ...
     * @return True if decoding rate is reached 
     * */
    public boolean readPktData(ByteArrayInputStream in, int l, int r, int c, int p, CBlkInfo[][][] cbI, byte[][][][] cbData, int[] nb) throws IOException
    {
        int curOff = 0;
        Coord curCB;
        CBlkInfo ccb;
        ...
        for (int s = mins; s < maxs; s++)
        {
            for (int numCB = 0; numCB < cblks[s].size(); numCB++)
            {
                ...
                //long skipped = in.skip(ccb.len[l]);
                int off = 0;
                byte[] data = cbData[s][cbc.y][cbc.x];
                
                if (data == null)                    
                    data = new byte[ccb.len[l]];
                
                else // grow byte array
                {
                    byte[] old = data;
                    data = new byte[old.length + ccb.len[l]];
                    System.arraycopy(old, 0, data, 0, old.length);
                    off = old.length;
                }
                
                int read = readFully(in, data, off, ccb.len[l]);
                System.out.println("  l: " + l + " - s: " + s + " - ccb: " + cbc.x + "," + cbc.y + ": " + read + " bytes read");
...
//                }
                ...
                // sync w/ reads on code block and set flag to READY
                synchronized (ccb)
                {
                    ccb.pktIdx[0] = READY;
                    ccb.notify();
    ...
    public boolean readSOPMarker(int p, int c, int r) throws IOException
    {
        int val;
        ...
        //        ehs.seek(pos);
        in.mark(2);
        if ((short) ((in.read() << 8) | in.read()) != Markers.SOP)
        {
            in.reset();
            return false;
        }
        ...
        readFully(in, sopArray, 0, Markers.SOP_LENGTH);
    ...
    private int readFully(InputStream in, byte[] buf, int offset, int length) throws IOException
    {
        int readBytes = 0;

        while (readBytes < length)
        {
            int read = in.read(buf, offset + readBytes, length - readBytes);
            if (read < 0)
                break;
            readBytes += read;
        }
